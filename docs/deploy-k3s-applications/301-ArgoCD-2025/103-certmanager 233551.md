...
helm update --install cert-manager jetstack/cert-manager --namespace cert-manager --create-namespace --set installCRDs=true --version v1.14.4

````

#### (alternate) install from local helm charts. 
```
# download Helm Chart
...
```
## EOF in new line
cat <<EOF > 01-ca-key-pair-k8s-lab.yaml
